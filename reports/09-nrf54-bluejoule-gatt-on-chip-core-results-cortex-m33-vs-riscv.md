...
FLPR stands for **fast lightweight peripheral processor**. In the nRF54L15, it is a constrained on-chip RISC-V coprocessor that can access much of the same peripheral set as the main application core.

The purpose is to show how the same BLE connection benchmark behaves when moved from the main Arm application core to the constrained on-chip RISC-V coprocessor.

This is a same-device-family comparison. The benchmark profile, central behavior, measurement workflow, and EM•Scope scoring path are the same as the existing BlueJoule-GATT reports.

The result is also a portability test. The FLPR target is not a separate hand-written BLE implementation. It reuses the same BlueJoule-GATT logic, the same portable BLE controller path, and most of the same nRF54 low-level driver code.

The question for this report is therefore narrow:
...
The existing BlueJoule-GATT nRF54 result runs on the Cortex-M33 application core. This report adds the RISC-V FLPR core as a second EM•Script execution target inside the same device family.
...
BlueJoule-GATT does not require a full general-purpose BLE stack. It requires a bounded BLE peripheral path: advertise, connect, complete targeted discovery, accept a `Command` write, serve a `Status` read, disconnect, and repeat.

A conventional BLE stack would normally be hard to imagine inside that kind of small SRAM-resident execution envelope. The EM•Script result changes that assumption.
...
The strategic question is therefore not whether FLPR should replace the Cortex-M33 as the normal BLE host. The question is whether tiny, profile-specialized BLE software can make this kind of secondary on-chip processor practical for a real connection benchmark.
...
The first target is the **Cortex-M33 application core**. This is the main application processor and is the target used for the existing nRF54 EM•Script BlueJoule-GATT result.

The second target is the **RISC-V FLPR core**. FLPR stands for **fast lightweight peripheral processor**. It is implemented as a VPR RISC-V processor inside the nRF54L15.

In Nordic / Zephyr target naming, these roughly correspond to:
...
The two targets are different processor architectures, but they are not isolated from the same-chip device environment. Both can access much of the same nRF54 peripheral set. That is important for this benchmark because the FLPR result does not require a separate hand-written radio stack or a separate device-driver universe.

The FLPR execution model is different from the application-core model. The FLPR image is packaged with an M33 launcher. The launcher performs the required setup, prepares the FLPR image, and starts the FLPR processor.
...
For this experiment, the FLPR image runs from SRAM and is packed into one retained 16 KB SRAM block. That block contains the benchmark code and runtime state needed for the FLPR side of the workload.

The important comparison is therefore:

```text
same nRF54 device family
same BlueJoule-GATT workload
same EM•Script source architecture
different execution core and instruction set
M33 inactive during FLPR benchmark execution
```

## 4. Shared Implementation

All EM•Script BlueJoule-GATT builds share a substantial amount of portable code.

That is part of the benchmark design. The application logic, GATT profile, portable BLE controller, connection-management logic, and benchmark transaction are intended to remain common as the workload moves from one target to another.

Normally, the largest target-specific differences appear lower in the stack: radio driver code, device-driver code, interrupt handling, clocks, timers, and low-level startup.
...
This makes the comparison stronger. The Cortex-M33 and FLPR builds are not two unrelated BLE implementations, and they are not two unrelated nRF54 driver stacks.

They are mostly the same EM•Script BLE workload and the same nRF54 device code compiled for two different execution cores.
...
The most important measurement condition is that the Cortex-M33 and FLPR results were taken back-to-back using the current EM•Script nRF54 codebase. That matters because the nRF54 EM•Script implementation continued to evolve during FLPR bring-up, and most of that code is shared between the two targets.

The earlier published Cortex-M33 score remains a useful historical result, but this report uses the back-to-back Cortex-M33 / FLPR pair for the same-chip comparison.
...
The result is therefore straightforward: with the current EM•Script codebase, BlueJoule-GATT scores the same on the nRF54 Cortex-M33 application core and the nRF54 RISC-V FLPR core.

## 7. Energy Breakdown

The score result is reflected directly in the underlying EM•Scope energy numbers.
...
The energy breakdown reinforces the main result: the two targets are effectively tied.

The representative event energy is the same within measurement resolution:

```text
Cortex-M33 application core:  129.6 µJ
RISC-V FLPR core:             129.7 µJ
```

The energy per 10 s scoring period is also the same:

```text
Cortex-M33 application core:  152.2 µJ
RISC-V FLPR core:             152.2 µJ
```

Sleep power is also in the same class:

```text
Cortex-M33 application core:  2.3 µW
RISC-V FLPR core:             2.3 µW
```

Because the score, event energy, period energy, and sleep power are all essentially identical, this report does not include a separate matched-event drilldown. Previous BlueJoule-GATT reports used event drilldowns when the traces exposed a clear software-path difference. Here, the measured result is the opposite: nothing material separates the two nRF54 EM•Script targets at the event-energy level.

That makes the interpretation simpler. For this benchmark and this codebase, moving the EM•Script BlueJoule-GATT workload from the Cortex-M33 application core to the RISC-V FLPR core does not produce a meaningful energy penalty.

## 8. Program Size and Retained SRAM
...
The reason this comparison is meaningful is the shared implementation. This is not a custom FLPR-only BLE stack and not a separate FLPR-only radio path. It is mostly the same EM•Script application, BLE controller, connection logic, and nRF54 device support compiled for two different instruction-set architectures.
...
That combination is the real finding:
...
## 10. RISC-V and Tiny-Code Architecture
